...
public class MultiPlayerMenu implements ActionListener, ItemListener,
	...
	public void buildMPMenu() {
		...
		Dimension widerLabelSize = new Dimension(widerLabelWIDTH
				* ClientGUI.SCALE, labelHEIGHT * ClientGUI.SCALE);
		Dimension textFieldSize = new Dimension(textFieldWIDTH
				* ClientGUI.SCALE, textFieldHEIGHT * ClientGUI.SCALE);
		Dimension buttonSize = new Dimension(buttonWIDTH * ClientGUI.SCALE,
				buttonHEIGHT * ClientGUI.SCALE);
		Dimension messageBoxSize = new Dimension(messageBoxWIDTH
				* ClientGUI.SCALE, messageBoxHEIGHT * ClientGUI.SCALE);
		Dimension checkBoxSize = new Dimension(checkBoxWIDTH * ClientGUI.SCALE,
				checkBoxHEIGHT * ClientGUI.SCALE);
		Dimension comboBoxSize = new Dimension(comboBoxWIDTH * ClientGUI.SCALE,
				comboBoxHEIGHT * ClientGUI.SCALE);
		Dimension scrollBarSize = new Dimension(scrollBarWIDTH
				* ClientGUI.SCALE, messageBoxHEIGHT * ClientGUI.SCALE);
		...
		userTField.setBounds((halfWIDTH - (textFieldWIDTH / 2))
				* ClientGUI.SCALE, SPACING * ClientGUI.SCALE,
				textFieldSize.width, textFieldSize.height);
		userLabel.setBounds((int) userTField.getBounds().getMinX() - SPACING
				* ClientGUI.SCALE - (labelWIDTH * ClientGUI.SCALE), SPACING
				* ClientGUI.SCALE, labelSize.width, labelSize.height);

		ipTField.setBounds(
				(halfWIDTH - (textFieldWIDTH / 2)) * ClientGUI.SCALE,
				(int) userTField.getBounds().getMaxY() + SPACING
						* ClientGUI.SCALE, textFieldSize.width,
				textFieldSize.height);
		ipLabel.setBounds((int) ipTField.getBounds().getMinX() - SPACING
				* ClientGUI.SCALE - (labelWIDTH * ClientGUI.SCALE),
				(int) userLabel.getBounds().getMaxY() + SPACING
						* ClientGUI.SCALE, labelSize.width, labelSize.height);

		connectButton.setBounds((int) ipTField.getBounds().getMaxX() + SPACING
				* ClientGUI.SCALE, (int) userLabel.getBounds().getMaxY()
				+ SPACING * ClientGUI.SCALE, buttonSize.width,
				buttonSize.height);

		portTField.setBounds((halfWIDTH - (textFieldWIDTH / 2))
				* ClientGUI.SCALE, (int) ipTField.getBounds().getMaxY()
				+ SPACING * ClientGUI.SCALE, textFieldSize.width,
				textFieldSize.height);
		portLabel
				.setBounds((int) portTField.getBounds().getMinX() - SPACING
						* ClientGUI.SCALE - (labelWIDTH * ClientGUI.SCALE),
						(int) ipLabel.getBounds().getMaxY() + SPACING
								* ClientGUI.SCALE, labelSize.width,
						labelSize.height);

		AIComboBox.setBounds((halfWIDTH - (textFieldWIDTH / 2))
				* ClientGUI.SCALE, (int) portTField.getBounds().getMaxY()
				+ SPACING * ClientGUI.SCALE, comboBoxSize.width,
				comboBoxSize.height);
		AILabel.setBounds((int) AIComboBox.getBounds().getMinX() - SPACING
				* ClientGUI.SCALE - (widerLabelWIDTH * ClientGUI.SCALE),
				(int) portLabel.getBounds().getMaxY() + SPACING
						* ClientGUI.SCALE, widerLabelSize.width,
				widerLabelSize.height);
		AICheckBox.setBounds((int) AIComboBox.getBounds().getMaxX() + SPACING
				* ClientGUI.SCALE, (int) portLabel.getBounds().getMaxY()
				+ SPACING * ClientGUI.SCALE, checkBoxSize.width,
				checkBoxSize.height);

		messageBox.setBounds(SPACING * ClientGUI.SCALE, (int) AIComboBox
				.getBounds().getMaxY() + SPACING * ClientGUI.SCALE,
				messageBoxSize.width, messageBoxSize.height);
		scrollMessageBox.setBounds((SPACING * ClientGUI.SCALE),
				(int) messageBox.getBounds().getMinY(), messageBoxSize.width
